...
You'll need to have [nox](https://github.com/theacodes/nox) and [uv](https://github.com/astral-sh/uv) installed:
...
We use [uv](https://github.com/astral-sh/uv) for managing dependencies and developing locally.
If you want to change any of the project requirements (or requirement bounds) in `pyproject.toml`,
make sure that `uv.lock` file reflects those changes by using `uv add`, `uv remove`, `uv lock`, or other
commands - see [documentation](https://docs.astral.sh/uv/). You can verify that the lock file is up to
date by running the linter.
